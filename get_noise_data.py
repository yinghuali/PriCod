...
ap.add_argument("--img_np_path", type=str, default='')
...
def run_img(img_path, select_method, save_path_pkl):
    ...
    img_np = pickle.load(open(img_path, 'rb'))
    index_list = list(range(len(img_np)))
    num_indices_to_extract = int(0.3 * len(index_list))
    random_indices = random.sample(index_list, num_indices_to_extract)
    for idx in random_indices:
        tmp_img = img_np[idx]
        tmp_img = method(tmp_img)
        img_np[idx] = tmp_img

    img_np = np.array(img_np, dtype='uint8')
    pickle.dump(img_np, open(save_path_pkl, 'wb'), protocol=4)


if __name__ == '__main__':
    run_img(img_np_path, select_method, save_path_pkl)
