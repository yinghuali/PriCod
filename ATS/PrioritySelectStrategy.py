...
from AbsSelectStrategy import AbsSelectStrategy
import pandas as pd
from util import get_data_by_label_with_idx
...
class PrioritySelectStrategy(AbsSelectStrategy):
    ...
    def get_max_coverage_sequence(self, Tx, Ty, n, M, base_path=None, use_add=False, is_save_ps=False, prefix="",
        ...
        for i in tqdm(range(n)):
            csv_data["label"] = i
            Tx_i, Ty_i, T_idx_arr , _idx_arr= get_data_by_label_with_idx(Tx, Ty, i)
            # Tx_prob_matrixc = M.predict(Tx_i)
            Tx_prob_matrixc = M[_idx_arr]
